...
{
    internal class ProgressObservable<T> : IProgressObservable<T>, IObserver<JsonElement>
    {
        private readonly Func<object, T> _factory;
        private readonly CompositeDisposable _disposable;
        private readonly ReplaySubject<JsonElement> _dataSubject;

        public ProgressObservable(ProgressToken token, Func<object, T> factory, Action disposal)
        {
            _factory = factory;
            _dataSubject = new ReplaySubject<JsonElement>(1, Scheduler.Immediate);
        ...
        public IDisposable Subscribe(IObserver<T> observer) =>
            _disposable.IsDisposed ? Disposable.Empty : _dataSubject.Select(value => _factory(value)).Subscribe(observer);
    }
}
