...
namespace OmniSharp.Extensions.JsonRpc
{
    ...
    internal static class Hex
    {
        ...
        {
            var bytes = MemoryMarshal.Cast<char, byte>(dest);

            // Inspired by http://stackoverflow.com/questions/623104/c-byte-to-hex-string/3974535#3974535
            var lengthInNibbles = src.Length * 2;

            for (var i = 0; i < ( lengthInNibbles & -2 ); i++)
            {
                var index0 = +i >> 1;
        ...
        internal static void Decode(ReadOnlySpan<char> value, Span<byte> bytes)
        {
            ...
            {
                var c1 = ReverseHexDigits[value[i++] - '0'] << 4;
    ...
    internal unsafe struct TraceParent
    {
        ...
        internal TraceParent(string? traceparent)
        {
            ...
            var traceparentChars = traceparent.AsSpan();

            // Decode version
            var slice = Consume(ref traceparentChars, VersionByteCount * 2);
        ...
        public override string ToString()
        {
            ...
            Span<char> traceparent =
                stackalloc char[( VersionByteCount * 2 ) + 1 + ( TraceIdByteCount * 2 ) + 1 + ( ParentIdByteCount * 2 ) + 1 + ( FlagsByteCount * 2 )];
            var traceParentRemaining = traceparent;
        ...
        private static Guid CopyBufferToGuid(ReadOnlySpan<byte> buffer)
        {
    ...
    public sealed class CorrelationManagerTracingStrategy : IActivityTracingStrategy
    {
        ...
        public unsafe void ApplyOutgoing(ITraceData data)
        {
            if (Trace.CorrelationManager.ActivityId != Guid.Empty)
            {
                ...
                if (TraceSource is { } && ( TraceSource.Switch.Level & SourceLevels.ActivityTracing ) == SourceLevels.ActivityTracing
                                       && TraceSource.Listeners.Count > 0)
                {
        ...
        public IDisposable? ApplyInbound(ITraceData request)
        {
            ...
            var activityName = request is IMethodWithParams p ? p.Method : null;

            return new ActivityState(request, TraceSource, activityName, traceparent.TraceIdGuid, childActivityId);
        }

        private static void FillRandomBytes(Span<byte> buffer)
        {
            CopyGuidToBuffer(Guid.NewGuid(), buffer);
        }

        private static unsafe void CopyGuidToBuffer(Guid guid, Span<byte> buffer)
        {
            var guidBytes = new ReadOnlySpan<byte>(&guid, sizeof(Guid));
        ...
        private class ActivityState : IDisposable
        {
            ...
            internal ActivityState(ITraceData request, TraceSource? traceSource, string? activityName, Guid parentTraceId, Guid childTraceId)
            {
                ...
                originalTraceState = TraceState;
                this.activityName = activityName ?? "<unnamed>";
                this.parentTraceId = parentTraceId;

                if (traceSource is { } && parentTraceId != Guid.Empty)
                {
            ...
            public void Dispose()
            {
                traceSource?.TraceEvent(TraceEventType.Stop, 0, activityName);

                if (parentTraceId != Guid.Empty)
                {
                    traceSource?.TraceTransfer(0, nameof(TraceEventType.Transfer), parentTraceId);
                }
