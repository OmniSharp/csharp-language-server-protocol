...
namespace Dap.Tests.Integration
{
    public class HandlersManagerIntegrationTests : DebugAdapterProtocolTestBase
    {
        ...
        public async Task Link_Should_Fail_If_Link_Is_On_The_Wrong_Side()
        {
            ...
            var handlersManager = server.GetRequiredService<IHandlersManager>();
            handlersManager.Add(
                ( Substitute.For(new[] { typeof(ICompletionsHandler) }, Array.Empty<object>()) as IJsonRpcHandler )!, new JsonRpcHandlerOptions()
            );
            ...
            a.Should().Throw<ArgumentException>().Which.Message.Should()
             .Contain($"Did you mean to link '{RequestNames.Completions}' to 'my/completions' instead");
        }
    }
}
