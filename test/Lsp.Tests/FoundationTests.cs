...
namespace Lsp.Tests
{
    public class FoundationTests
    {
        ...
        private class DebuggerDisplayTypes : TheoryData<Type>
        {
            public DebuggerDisplayTypes()
            {
                foreach (var item in typeof(DocumentSymbol).Assembly.ExportedTypes
                                                           ...
                                                           .Where(
                                                                z => z.GetCustomAttributes<DebuggerDisplayAttribute>()
                                                                      .Any(x => x.Value.StartsWith("{DebuggerDisplay"))
                                                            )
        ...
        )
        {
            ...
            extensionClass.GetMethods().Any(z => z.Name == onMethodName && typeof(IJsonRpcHandlerRegistry).IsAssignableFrom(z.GetParameters()[0].ParameterType))
                          .Should()
                          .BeTrue($"{descriptor.HandlerType.FullName} is missing event extension methods named {onMethodName}");
            extensionClass.GetMethods().Any(z => z.Name == sendMethodName && typeof(IResponseRouter).IsAssignableFrom(z.GetParameters()[0].ParameterType))
                          .Should()
        ...
        )
        {
            ...
            {
                Direction.ClientToServer => new (string type, Func<ParameterInfo, bool> matcher)[]
                    { ( "Server", info => info.ParameterType.Name.EndsWith("ServerRegistry") ) },
                Direction.ServerToClient => new (string type, Func<ParameterInfo, bool> matcher)[]
                    { ( "Client", info => info.ParameterType.Name.EndsWith("ClientRegistry") ) },
            ...
            {
                Direction.ClientToServer => new (string type, Func<ParameterInfo, bool> matcher)[]
                    { ( "Server", info => info.ParameterType.Name.EndsWith("Client") ) },
                Direction.ServerToClient => new (string type, Func<ParameterInfo, bool> matcher)[]
                    { ( "Client", info => info.ParameterType.Name.EndsWith("Server") ) },
                Direction.Bidirectional => new (string type, Func<ParameterInfo, bool> matcher)[]
        ...
        )
        {
...

            {
                ...
                if (descriptor.IsRequest && TypeHandlerExtensionData.HandlersToSkip.All(z => descriptor.HandlerType != z))
                {
                    ...
                    matcher.Match(
                        $"Func<{descriptor.ParamsType.Name}, CancellationToken, {returnType.Name}>", isFunc, takesParameter, containsCancellationToken, returns
                    );
                    if (descriptor.HasCapability)
                    {
                        matcher.Match(
                            $"Func<{descriptor.ParamsType.Name}, {descriptor.CapabilityType!.Name}, CancellationToken, {returnType.Name}>", isFunc,
                            takesParameter,
                    ...
                    {
                        var capability = ForAnyParameter(
                            info => info.ParameterType.GetGenericArguments().Skip(2).FirstOrDefault() == descriptor.CapabilityType
                        );
                        var observesPartialResultType = ForAnyParameter(
                            info =>
                                info.ParameterType.GetGenericArguments().Skip(1).FirstOrDefault()
                             == typeof(IObserver<>).MakeGenericType(descriptor.PartialItemType!)
                        );

                        matcher.Match(
                            $"Action<{descriptor.ParamsType.Name}, IObserver<{descriptor.PartialItemType!.Name}>>", isAction, takesParameter,
                ...
                if (descriptor.IsNotification)
                {
                    ...
                    if (descriptor.HasCapability)
                    {
                        ...
                        matcher.Match(
                            $"Action<{descriptor.ParamsType.Name}, {descriptor.CapabilityType.Name}, CancellationToken>", isAction, takesParameter,
                            takesCapability,
            ...
            }
            {
                ...
                if (descriptor.IsRequest && descriptor.HasPartialItems)
                {
                    Func<MethodInfo, bool> partialReturnType = info =>
                        typeof(IRequestProgressObservable<,>).MakeGenericType(
                                                                  typeof(IEnumerable<>).MakeGenericType(descriptor.PartialItemsType!), descriptor.ResponseType!
                                                              )
                ...
                {
                    Func<MethodInfo, bool> partialReturnType = info =>
                        typeof(IRequestProgressObservable<,>).MakeGenericType(descriptor.PartialItemType!, descriptor.ResponseType!)
                                                             .IsAssignableFrom(info.ReturnType);
                    matcher.Match(
                ...
                {
                    matcher.Match(
                        $"Func<{descriptor.ParamsType!.Name}, CancellationToken, {returnType.Name}>", takesParameter, containsCancellationToken, returns
                    );
                }
                    ...
                    foreach (var method in methods)
                    {
                        if (method.Name == GetOnMethodName(_descriptor))
                        {
                            ...
                            registrySub.Received().ReceivedCalls()
                                       .Any(
                                            z => z.GetMethodInfo().Name
                                              == nameof(IJsonRpcHandlerRegistry<IJsonRpcHandlerRegistry<IJsonRpcServerRegistry>>.AddHandler) &&
        ...
        public class ParamsShouldHaveMethodAttributeData : TheoryData<Type>
        {
            public ParamsShouldHaveMethodAttributeData()
            {
                foreach (var type in typeof(CompletionParams).Assembly.ExportedTypes.Where(
                             z =>
                                 z.IsClass && !z.IsAbstract && z.GetInterfaces().Any(
                                     x => x.IsGenericType && typeof(IRequest<>).IsAssignableFrom(x.GetGenericTypeDefinition())
                                 )
                         ))
                {
                    Add(type);
            ...
            public TypeHandlerExtensionData()
            {
                        ...
                        )
                {
                    ...
                    if (type == typeof(ISemanticTokensFullHandler) || type == typeof(ISemanticTokensDeltaHandler)
                                                                   || type == typeof(ISemanticTokensRangeHandler)) continue;
